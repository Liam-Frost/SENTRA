...
export default function ServerFleet({ state }: ServerFleetProps) {
  const serverIds = Object.keys(state.servers) as ServerId[];
  const coolingCount = serverIds.filter((id) => state.servers[id]?.cooling).length;
    ...
    <div className="page">
      <div className="page-header">
        ...
        <div className="page-status">
          ...
          <div className="status-chip">Cooling {coolingCount}/{serverIds.length || 0}</div>
        </div>
      </div>

      {serverIds.length === 0 ? (
        <div className="card empty">No registered nodes yet. Install probe agents to start monitoring.</div>
      ) : (
